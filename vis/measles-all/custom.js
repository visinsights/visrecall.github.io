...
function handleLoadComplete(percent, pop, i, idx, rZero = 15, interactive = false) {
	...
	if (interactive){
		updateIntPopBar(pop, percent);
	} else {
		updatePopBar(pop, percent);
	}
	return pop.infected
}
...
function initPopBar(pop, percentVacc){
	...
	d3.select('.bar._'+percentVacc+'.susceptible')
		.attr("style", "width: "+(pop.susceptible)*2+"px");

	d3.select('.bar._'+percentVacc+'.vaccinated')
...
function updatePopBar(pop, percentVacc){
